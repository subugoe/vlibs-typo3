...
function expand( id ) {
			...
			//var cat = $("#n0" + id + " a:first").text();
			if (id.length < 6){
			$("#searchMsc").fadeIn("slow");
			$("#cat").html('<em>'+id+'</em>');
			...
			$("#searchMsc").fadeOut("normal");
			}
			//fetch document
			
			$.get("fileadmin/browsing/nodetest/" + id + ".txt",function(data){
				...
				//alert(id);
				$('#n1' + id).children("a:first").attr("class", "plus");
				var offset = $('#n1' + id).offset();
				$("#searchMsc").animate( { top:offset.top }, 600);
				...
				$('#n1' + id).children("ul").children("li").each(function() {
					...
					if ($(this).attr("id").substr(0,1) == "l"){
						...
						$(this).children("a:first").click(function(event){ 	
							...
							$("#cat").html('<em>'+$(this).parent().attr("id").substr(2)+'</em>');

 							$("#msc").attr("value", $(this).parent().attr("id").substr(2));
							//highlight actual category
							$(".plus").removeClass("plus");
					...
					reg2expand($(this).attr("id").substr(2));
...
function reg2expand(id) {
	...
	$('#n0' + id).children("a[class!='docs']").each(function() {
...
function reg2contract(id) {
	...
	$('#n1' + id).children("a:first").each(function() {
		$(this).click(function(event){
		//on click...
			event.preventDefault();
...
function contract( id ) {
			...
			*/
			...
			$("#n1" + id).children("ul").slideUp("medium",function(){
				$(this).remove();
...
$(document).ready(function(){
		//alert("jquery is starting");
		$("#searchMsc").css("display", "none");
		$("#tree ul").children("li").each( function() {
			
			var id = $(this).attr("id").substr(2);
			//alert(id);
			reg2expand(id);
			//$("#searchMsc").css("top","50");
