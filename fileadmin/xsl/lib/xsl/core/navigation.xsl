...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:options">
        <div id="ds-options">
            <h3 id="ds-search-option-head" class="ds-option-set-head"><xsl:value-of select="/dri:document/dri:i18n/dri:message[@key='xmlui.dri2xhtml.structural.search']"/></h3>
    ...
    <xsl:variable name="auth"><xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:userMeta/@authenticated"/></xsl:variable>
    <!-- TODO: figure out why i18n tags break the go button -->
    <xsl:template match="dri:options">
        <div id="ds-options-wrapper">
            <div id="ds-options">
                <h1 id="ds-search-option-head" class="ds-option-set-head">
                    <xsl:value-of select="/dri:document/dri:i18n/dri:message[@key='xmlui.dri2xhtml.structural.search']"/>
                </h1>
                <div id="ds-search-option" class="ds-option-set">
                    <!-- The form, complete with a text box and a button, all built from attributes referenced
                 from under pageMeta. -->
                    <form id="ds-search-form" method="post">
                        ...
                        <fieldset>
			    <span><xsl:value-of select="/dri:document/dri:i18n/dri:message[@key='xmlui.static.fulltext.search']"/></span>
                            <input class="ds-text-field " type="text">
                                <xsl:attribute name="name">
                                    <xsl:value-of
                            ...
                            <xsl:if test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='focus'][@qualifier='container']">
                                <label>
                                    <input id="ds-search-form-scope-all" type="radio" name="scope" value=""
                                           checked="checked"/>
                                    <xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.dri2xhtml.structural.search']"/>
                                </label>
                                <br/>
                                <label>
                                    <input id="ds-search-form-scope-container" type="radio" name="scope">
                                    ...
                                    <xsl:choose>
                                        <xsl:when
                                            ...
                                            <xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.dri2xhtml.structural.search-in-community']"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.dri2xhtml.structural.search-in-collection']"/>
                                        </xsl:otherwise>
                    ...
                    <a>
                        ...
                        <xsl:value-of select="/dri:docuemnt/i18n/message[@key='mlui.dri2xhtml.structural.search-advanced']"/>
                    </a>
                </div>

                <!-- Once the search box is built, the other parts of the options are added -->
		<!-- Do not show MyAccount for unauthenticated user -->
		<xsl:choose>
			<xsl:when test="/dri:document/dri:meta/dri:userMeta/@authenticated='no'">
				<xsl:for-each select="//dri:options/dri:list">
				<xsl:if test="(@id != 'aspect.viewArtifacts.Navigation.list.account')">
			                <xsl:apply-templates/>
				</xsl:if>
				</xsl:for-each>
			</xsl:when>
		...
		<h1 class="ds-option-set-head"><xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.static.navigation.help-head']"/></h1>
		<div class="ds-option-set">
		<ul class="ds-simple-list">
		<li>
			<a href="/deposit-licence"><xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.static.navigation.deposit-licence']"/></a>
		</li>
		<li>
			<a href="/help-publication"><xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmluii.static.navigation.help-publication']"/></a>
                </li>
                <li>
                        <a href="/guides"><xsl:value-of select="/dri:docuemnt/i18n/message[@key='xmlui.guides']"/></a>
                </li>
		</ul>
...
<!--    <xsl:template match="dri:options/dri:list/dri:list"  mode="redirect">
        <li>
            <xsl:apply-templates select="dri:head" mode="nested"/>
            <ul class="ds-simple-list sublist">
		        <a>
		...
		<xsl:otherwise>
		<xsl:attribute name="href"><xsl:value-of select="adwportal.sub.uni-goettingen.de/digitale-bibliothek/res-doctae-dokumentenserver"/></xsl:attribute>
		</xsl:otherwise>
		</xsl:choose>
            </xsl:if>
