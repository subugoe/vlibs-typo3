...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
   ...
   <!--<xsl:template match="dri:help">
    ...
    <xsl:template match="dri:field[@type='composite'][dri:field/dri:instance | dri:params/@operations]" mode="formComposite" priority="2">
        <xsl:variable name="confidenceIndicatorID" select="concat(translate(@id,'.','_'),'_confidence_indicator')"/>
	<!-- M.M.Submit button value is not internationalized! -->
	<xsl:variable name="add">
		<xsl:choose>
			<xsl:when test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='locale'] = 'de'">
				<xsl:text>Filter anwenden</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>Apply filter</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
        <div class="ds-form-content">
	...
	<xsl:if test="not(@id='aspect.discovery.SimpleSearch.field.submit')">
        	<xsl:apply-templates select="." mode="normalField"/>
	</xsl:if>
	<xsl:if test="@id='aspect.discovery.SimpleSearch.field.query'">

		 <xsl:apply-templates select="//dri:field[@id='aspect.discovery.SimpleSearch.field.submit']" mode="normalField"/>	
		 <input type="reset" value="reset" onclick="javascript:resetSearch();" name="reset" class="ds-button-field"/>
	</xsl:if>
        <xsl:if test="contains(dri:params/@operations,'add')">
            <!-- Add buttons should be named "submit_[field]_add" so that we can ignore errors from required fields when simply adding new values-->
            <input type="submit" value="Add" name="{concat('submit_',@n,'_add')}" class="ds-button-field ds-add-button">
              ...
              <xsl:if test="dri:params/@choicesPresentation = 'lookup'">
                <xsl:attribute name="style">
                  <xsl:text>display:none;</xsl:text>
	                ...
	                <!-- M.M.: customizeation for classification tree DDC -->
            <xsl:if test="@n='dc_subject_ddc'">
                 <br />
                 <a>
                 <xsl:attribute name="href"><xsl:text>javascript:void(null)</xsl:text></xsl:attribute>
                 <xsl:attribute name="onclick"><xsl:text>javascript:popUp("/controlledvocabulary-ddc.html")</xsl:text>    </xsl:attribute>
                 <span class="controlledVocabularyLink"><i18n:text>xmlui.mirage.core.form.ddc.label</i18n:text></span>
                 </a>
          </xsl:if>
              <!-- M.M. end -->
