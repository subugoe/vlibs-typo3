...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
	...
	<!-- M.M. -->
	<xsl:variable name="temp"><xsl:value-of select="substring-after(@url, '/metadata/handle/')"/></xsl:variable>
	<xsl:variable name="id"><xsl:value-of select="substring-before($temp, '/mets.xml')"/></xsl:variable>
	<!-- ends -->
	    ...
	    <!-- M.M. Hihglighting results -->
	    <xsl:if test="//dri:list[@n='search-highlights-repository']">
	
		<xsl:if test="//dri:list[@n='search-highlights-repository']/dri:list[@n=$id]">		
		<br />
			<xsl:for-each select="//dri:list[@n='search-highlights-repository']/dri:list[@n=$id]/dri:item">
