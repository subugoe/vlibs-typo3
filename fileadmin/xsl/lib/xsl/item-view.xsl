...
<xsl:stylesheet
    ...
    exclude-result-prefixes="xalan encoder i18n dri mets dim  xlink xsl atom ore oreatom util">

    <xsl:output indent="yes"/>

    <!-- global variables -->
    <xsl:variable name="baseURL">http://adw-digital.sub.uni-goettingen.de/</xsl:variable>
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
          ...
          <xsl:when test="$clause = 5 and (dim:field[@element='description' and @qualifier='tableofcontents'])">
                   <div class="simple-item-view-other">
		   <span class="bold"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-toc</i18n:text>:</span>
			...
			<xsl:otherwise>
				<span>
				<xsl:copy-of select="dim:field[@element='description' and @qualifier='toc']"/>
				</span>
			</xsl:otherwise>	
			</xsl:choose>
		   </div>
    ...
    <xsl:template match="mets:file">
        ...
        <div class="file-wrapper clearfix">
	     ...
	     <xsl:choose>
	     <xsl:when test="/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field[@element='relation' and @qualifier='dfgviewer']">
                <a>
                    <xsl:attribute name="href">
		    ...
		    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-files-dfg-viewOpen</i18n:text>
                </a>
	     ...
	     <xsl:otherwise>
                <a>
                    <xsl:attribute name="href">
                        <xsl:value-of select="concat($googleViewURL, $context-path, mets:FLocat[@LOCTYPE='URL']/@xlink:href)"/>
                    </xsl:attribute>
		    <xsl:attribute name="target"><xsl:text>_blank</xsl:text></xsl:attribute>
                    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-files-google-viewOpen</i18n:text>
                </a>
	      </xsl:otherwise>
	      </xsl:choose>
		<xsl:text>&#160;&#160;</xsl:text>
