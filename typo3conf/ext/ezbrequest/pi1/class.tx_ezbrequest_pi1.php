...
class tx_ezbrequest_pi1 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		$listParams['notation'] = $this->conf['notation'];                                                                                                                                                                $itemParams = $this->baseParams;
	...
	* @param string			$itemParamString: $itemParams as string
	...
	function createItemTable($journal, $listParams, $listParamString, $itemParam, $itemParamString) {
		...
		foreach ($journal->detail->homepages->homepage as $homepage) 
			...
			if (strlen($homepage) > 50) { $moreValues .= $this->pi_linkToPage(substr($homepage, 0, 50).'...', $homepage, '_blank', $empty).'<br/>';}
			else {
		...
		if ($journal->detail->first_fulltext_issue) 
			{	if($journal->detail->first_fulltext_issue->first_volume) {$moreValues .= 'Vol. ' . $journal->detail->first_fulltext_issue->first_volume;}
				if($journal->detail->first_fulltext_issue->first_issue){$moreValues .= ', ' . $journal->detail->first_fulltext_issue->first_issue;}
				if($journal->detail->first_fulltext_issue->first_date){$moreValues .= ' ('.$journal->detail->first_fulltext_issue->first_date.')';};
				$itemDetails['first_fulltext_issue'] = $moreValues;
				$moreValues = "";
			}

		if ($journal->detail->last_fulltext_issue) 
			{	if($journal->detail->last_fulltext_issue->last_volume) {$moreValues .= 'Vol. ' . $journal->detail->last_fulltext_issue->last_volume;}
				if($journal->detail->last_fulltext_issue->last_issue){$moreValues.= ', ' . $journal->detail->last_fulltext_issue->last_issue;}
				if($journal->detail->last_fulltext_issue->last_date){$moreValues .= ' ('.$journal->detail->last_fulltext_issue->last_date.')';};
				$itemDetails['last_fulltext_issue'] = $moreValues;
				$moreValues = "";
			}

		if($journal->detail->appearence){$itemDetails['appearence'] = $journal->detail->appearence;}
		if($journal->detail->costs){$itemDetails['costs'] = $journal->detail->costs;}
		if($journal->detail->remarks){$itemDetails['remarks'] = $journal->detail->remarks;}
