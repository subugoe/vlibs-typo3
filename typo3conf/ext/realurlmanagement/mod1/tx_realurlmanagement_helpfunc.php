 ...
 *
 *
 *   57: class tx_realurlmanagement_helpfunc extends t3lib_SCbase
 *   73:     function encodePageName($title,$table='pagePath')
 *  105:     function getSpaceCharacter($tableName)
 *  135:     function searchForSpaceCharacter($array)
 *  155:     function decodePageName($title)
 *  169:     function getOrderBy($fields,$orderBy,$orderAscDesc,$additionalUrl)
 *  198:     function getDateTime($date,$format='d. m. Y - H:i:s')
 *  217:     function extGetTreeList($id,$depth,$begin = 0,$perms_clause)
 *  247:     function writeJSForDateTimeValidation()
 *  252:     function typo3FormFieldSet(theField, evallist, is_in, checkbox, checkboxValue)
 *  272:     function typo3FormFieldGet(theField, evallist, is_in, checkbox, checkboxValue, checkbox_off, checkSetValue)
 *  299:     function changeExpireDate($tablename, $where, $expireField)
 *  335:     function getLangArray()
 *  358:     function getLanguage($language,$langArr)
 *  372:     function setArrayTo_GP($array)
 *  382:     function getArrayFrom_GP($string)
 *  392:     function getPageBrowser($pageBrowser)
 *  454:     function makeLog($message,$type)
 *
 * TOTAL FUNCTIONS: 17
...
class tx_realurlmanagement_helpfunc extends t3lib_SCbase {
	...
	/**
	 * this function write the logentry
	 *
	 * @param	string		table for alias
	 * @param	[type]		$type: ...
	 * @return	string		space character
	 */
	function makeLog($message,$type){
		//do something
		$row['userid']=$GLOBALS['BE_USER']->user["uid"];
		$row['action']=0;
		$row['recuid']=0;
		$row['tablename']='';
		$row['recpid']=0;
		...
		$row['type']=33;
		$row['details_nr']=0;
		$row['IP']=t3lib_div::getIndpEnv('REMOTE_ADDR');
		$row['log_data']='';
		$row['event_pid']=-1;
		$row['workspace']=-99;
		$row['NEWid']='';
		$GLOBALS['TYPO3_DB']->exec_INSERTquery('sys_log',$row);
	}
}
