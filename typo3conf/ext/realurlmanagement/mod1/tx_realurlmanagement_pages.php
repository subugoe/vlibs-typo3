 ...
 *  210:     function editwholepage(wholepage,segment)
 *  368:     function getPagesTreeImages($data,$data_nach)
 *  393:     function showModule()
 *
 * TOTAL FUNCTIONS: 5
...
',$cells);
	...
	function showModule(){
		...
		if(($this->action=='delete')&&($this->pObj->perms_pages_delete)){
			if(($this->action_pageid!=0)&&($this->action_cacheid!=0)&&($this->action_hash!='')){
				...
				$res_log=$GLOBALS['TYPO3_DB']->exec_SELECTquery(
					'*',
					'tx_realurl_pathcache',
					'cache_id='.$this->action_cacheid
				);
				if($GLOBALS['TYPO3_DB']->sql_num_rows($res_log)>0){
					while($row_log=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_log)){
						$this->helpfunc->makeLog(str_replace(array('###OLD_NAME###','###UID###','###LANGUAGE###'),array($row_log['pagepath'],$row_log['page_id'],$row_log['language_id']),$LANG->getLL('login_delete',1)),'delete');
					}
				}
				/* make log end*/
		...
		if(($this->action=='deleteshown')&&($this->pObj->perms_pages_deleteShown)&&($this->action_showcacheid!='')){
			...
			$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_urldecodecache','page_id IN('.$delete_uids.')');
			$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_urlencodecache','page_id IN('.$delete_uids.')');
		};
		/* delete showns end */
				...
				if($newTitle1!=''){
					if(($action_wholePage=='YES' && $newTitle1!=t3lib_div::_GP('act_wholepage') && $this->pObj->perms_pages_editWholeURL) || ($action_wholePage!='YES' && $newTitle1!=t3lib_div::_GP('act_segment'))){
						...
						if($action_wholePage!='YES'){
							$oldTitel=t3lib_div::_GP('page_before').t3lib_div::_GP('act_segment');
						...
						if($GLOBALS['TYPO3_DB']->sql_num_rows($res_num) >0 ){
							...
							/* make log begin */
							$this->helpfunc->makeLog(str_replace(array('###OLD_NAME###','###UID###','###LANGUAGE###','###NEW_NAME###'),array($oldTitel,$this->action_pageid,$this->action_langid,$newTitle),$LANG->getLL('login_update',1)),'edit');
							/* make log end*/
		...
		if($this->pObj->MOD_SETTINGS['depth']==0){
			...
			return $return_content.'<input type="hidden" name="showcacheid" value="'.$this->shownCacheId.'" /><input type="hidden" value="" id="act2" name="act2" />';
		}
